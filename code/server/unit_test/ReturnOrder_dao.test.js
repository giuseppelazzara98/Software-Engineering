...
async function testgetReturn(returned) {
  test("getReturn", async () => {
    ...
    const itemId = await idao.postItem(
      1,
      "official supply for acer aspire",
      250,
      sku1.id,
      user.id
    );

    const order2 = await rdao.insertRO("2023/09/05 09:33", user.id, [
      {
        SKUId: sku1.id,
    ...
    const skuItems = [
      { SKUId: sku1.id, rfid: "01234567890523456789012345678901" },
